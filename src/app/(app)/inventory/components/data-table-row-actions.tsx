...
function EditItemForm({ item, setOpen }: { item: Good, setOpen: (open: boolean) => void }) {
  ...
  const [compositionTypes, setCompositionTypes] = useState<CompositionType[]>([]);
   ...
   useEffect(() => {
    if (!db) return;
    const clothingTypesRef = collection(db, 'clothing_types');
    const unsubClothing = onSnapshot(clothingTypesRef, (snapshot) => {
      setClothingTypes(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as ClothingType)));
    },
    (error) => {
        errorEmitter.emit('permission-error', new FirestorePermissionError({
            path: clothingTypesRef.path,
            operation: 'list',
        }));
    });

    const compositionTypesRef = collection(db, 'composition_types');
    const unsubComposition = onSnapshot(compositionTypesRef, (snapshot) => {
      setCompositionTypes(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as CompositionType)));
    },
    (error) => {
        errorEmitter.emit('permission-error', new FirestorePermissionError({
            path: compositionTypesRef.path,
            operation: 'list',
        }));
    });
    ...
    <>
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        ...
        <DialogContent className="sm:max-w-2xl">
            ...
            <EditItemForm item={item} setOpen={setIsEditDialogOpen} />
        </DialogContent>
      </Dialog>
